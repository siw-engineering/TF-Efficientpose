...
def draw_obj(raw_image, rotation, translation_vector, camera_matrix, plydata=None):
    #obj = 'D:\dataset\Linemod_preprocessed\models\obj_09.ply'
    #plydata = PlyData.read(obj)
    xlist = plydata['vertex']['x']
    ylist = plydata['vertex']['y']
    zlist = plydata['vertex']['z']
    xhy = np.array(list(zip(xlist, ylist, zlist)))
    points_bbox_2D_xhy, jacobian = cv2.projectPoints(xhy, rotation, translation_vector, camera_matrix, None)
...
def draw_detections(image, boxes, scores, labels, rotations, translations, class_to_bbox_3D, camera_matrix, color = None, label_to_name = None, score_threshold = 0.5, draw_bbox_2d = False, draw_name = False,
    ...
    for i in selection:
        ...
        image = draw_obj(image, rotations[i, :], translation_vector, camera_matrix, plydata=plydata)
...
def draw_annotations(image, annotations, class_to_bbox_3D, camera_matrix, color = (0, 255, 0), label_to_name = None, draw_bbox_2d = False, draw_name = False, plydata=None):
    ...
    for i in range(annotations['bboxes'].shape[0]):
        ...
        image = draw_obj(image, annotations['rotations'][i, :3], annotations['translations'][i, :], camera_matrix, plydata=plydata)
